...
struct FInfluenceMapNode {
	struct FVector NodeLocation; // 0x00(0x18)
	struct TArray<struct FLayerValue> LayerValueMap; // 0x18(0x10)
	struct TArray<struct FNodeConnectionInfo> NodeConnectionInfo; // 0x28(0x10)
};

// ScriptStruct AISpatialAwareness.InfluenceMapQuery
// Size: 0x18 (Inherited: 0x00)
struct FInfluenceMapQuery {
	struct FVector SourcePosition; // 0x00(0x18)
};
...
struct FNodeInfo {
	struct FVector Location; // 0x00(0x18)
	struct AMapMarkupWaypoint* Waypoint; // 0x18(0x08)
	struct TArray<struct FNeighboringNodeInfo> NeighboringNodesInfo; // 0x20(0x10)
};
