...
struct FSpriteDrawCallRecord {
	...
	struct UTexture* BaseTexture; // 0x18(0x08)
	char pad_20[0x30]; // 0x20(0x30)
	struct FColor Color; // 0x50(0x04)
	char pad_54[0xec]; // 0x54(0xec)
};
...
struct FSpriteGeometryShape {
	...
	float Rotation; // 0x38(0x04)
	bool bNegativeWinding; // 0x3c(0x01)
...
struct FSpriteInstanceData {
	...
	struct UPaperSprite* SourceSprite; // 0x80(0x08)
	struct FColor VertexColor; // 0x88(0x04)
	int32_t MaterialIndex; // 0x8c(0x04)
};

// ScriptStruct Paper2D.PaperSpriteSocket
// Size: 0x70 (Inherited: 0x00)
struct FPaperSpriteSocket {
	struct FTransform LocalTransform; // 0x00(0x60)
